...
#ifdef _WIN32
// ---------------------------------------------------------------------------
// Win32 WndProc subclass — handles borderless window chrome
// ---------------------------------------------------------------------------
static LRESULT CALLBACK TwistedWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    auto* window   = static_cast<Twisted::Window*>(GetPropA(hwnd, "TWISTED_WINDOW"));
    auto* prevProc = reinterpret_cast<WNDPROC>(GetPropA(hwnd, "TWISTED_PREV_WNDPROC"));
    ...
    case WM_NCHITTEST:
    {
        ...
        const bool left   = !maximized && cursor.x < borderSize;
        const bool right  = !maximized && cursor.x >= client.right  - borderSize;
        const bool top    = !maximized && cursor.y < borderSize;
        const bool bottom = !maximized && cursor.y >= client.bottom - borderSize;
        ...
        if (window)
        {
            const int tbHeight = window->GetTitleBarHeight();
            if (tbHeight > 0 && cursor.y < tbHeight)
            {
                const int btnAreaWidth = tbHeight * 3;
    ...
    case WM_NCCALCSIZE:
        // When maximized, make the client area fill the entire window rect so the
        // WS_THICKFRAME non-client border doesn't leave a thin strip at the edges.
    ...
    case WM_SETCURSOR:
    {
        // For non-client areas (resize handles, caption) let DefWindowProc set
        // the correct resize/arrow cursor. GLFW would override it to arrow otherwise.
    ...
    : m_windowsService(service)
{
    ...
    m_glfwWindow = glfwCreateWindow(size.x, size.y, title.c_str(), nullptr, nullptr);
    if (!m_glfwWindow)
    {
    ...
    glfwSetWindowUserPointer(m_glfwWindow, service);

#ifdef _WIN32
    {
        HWND hwnd = glfwGetWin32Window(m_glfwWindow);
        SetPropA(hwnd, "TWISTED_WINDOW", static_cast<HANDLE>(this));
        auto* prev = reinterpret_cast<WNDPROC>(SetWindowLongPtrW(hwnd, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(TwistedWndProc)));
        SetPropA(hwnd, "TWISTED_PREV_WNDPROC", reinterpret_cast<HANDLE>(prev));

        // WS_THICKFRAME is required for resize hit-testing to trigger the OS
        // resize modal loop. GLFW undecorated windows use WS_POPUP which may
        // omit it depending on the GLFW version.
    ...
    glfwSetDropCallback(m_glfwWindow,
        [](GLFWwindow* w, int count, const char** paths)
        {
            ...
            svc->DispatchEvent(Twisted::FilesDroppedEvent{ std::move(filePaths) });
        });
...
Window::Window(Window&& other) noexcept
    ...
    , m_windowsService(std::exchange(other.m_windowsService, nullptr))
    , m_titleBarHeight(other.m_titleBarHeight)
{}
...
Vec2i Window::GetSize() const
{
    ...
    Vec2i s{};
    glfwGetWindowSize(m_glfwWindow, &s.x, &s.y);
    return s;
}
...
Vec2i Window::GetClientSize() const
{
    if (!m_glfwWindow) return {};
    Vec2i s{};
    glfwGetFramebufferSize(m_glfwWindow, &s.x, &s.y);
    return s;
}
...
Vec2i Window::GetPosition() const
{
    if (!m_glfwWindow) return {};
    Vec2i p{};
    glfwGetWindowPos(m_glfwWindow, &p.x, &p.y);
    return p;
}
...
{
    if (!m_glfwWindow) return {};
    const char* t = glfwGetWindowTitle(m_glfwWindow);
    return t ? t : "";
}
...
{
    if (m_glfwWindow)
        glfwSetWindowTitle(m_glfwWindow, newName.c_str());
}
...
{
    if (m_glfwWindow)
        glfwSetWindowPos(m_glfwWindow, newPosition.x, newPosition.y);
}
...
void Window::Minimize()
{
    if (!m_glfwWindow) return;
#ifdef _WIN32
    PostMessageW(glfwGetWin32Window(m_glfwWindow), WM_SYSCOMMAND, SC_MINIMIZE, 0);
#else
...
void Window::Maximize()
{
    if (!m_glfwWindow) return;
#ifdef _WIN32
    ShowWindow(glfwGetWin32Window(m_glfwWindow), SW_MAXIMIZE);
#else
...
void Window::Restore()
{
    if (!m_glfwWindow) return;
#ifdef _WIN32
    ShowWindow(glfwGetWin32Window(m_glfwWindow), SW_RESTORE);
#else
...
void Window::SetFullScreen(bool isFullScreen)
{
    ...
    else
    {
        ...
        Vec2i size = GetSize();
        glfwSetWindowMonitor(m_glfwWindow, nullptr,
