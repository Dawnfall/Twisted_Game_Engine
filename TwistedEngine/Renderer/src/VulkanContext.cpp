...
{
    static VulkanContext s_ctx(window);
    return s_ctx;
}
...
void VulkanContext::CreateSwapchain(uint32_t width, uint32_t height)
{
    ...
    // UNORM: ImGui colors are already sRGB-space; the monitor's gamma curve handles them.
    // Switch to SRGB + linearize inputs when implementing a linear PBR pipeline.
...
// -----------------------------------------------------------------------
// Offscreen render pass — R8G8B8A8_UNORM + D24S8, shared by camera FBs and shader pipelines.
// Created once and never destroyed during swapchain rebuilds.
// -----------------------------------------------------------------------
...
{
    // Compute an aligned slot stride so each camera's MVP data meets
    // minUniformBufferOffsetAlignment (required for VkDescriptorBufferInfo.offset).
    VkPhysicalDeviceProperties devProps{};
    vkGetPhysicalDeviceProperties(PhysicalDevice, &devProps);
    const VkDeviceSize alignment  = devProps.limits.minUniformBufferOffsetAlignment;
    const VkDeviceSize mvpBase    = 144;   // sizeof(ShaderMVPBuffer)
    MvpSlotStride = alignment > 1 ? (mvpBase + alignment - 1) & ~(alignment - 1) : mvpBase;

    const VkDeviceSize mvpSize    = MvpSlotStride * MaxCamerasPerFrame;
...
{
    // MaxCamerasPerFrame global sets per frame, each with 2 UBO bindings (MVP + Lights).
    const uint32_t globalSets = static_cast<uint32_t>(FramesInFlight * MaxCamerasPerFrame);

    std::array<VkDescriptorPoolSize, 2> poolSizes{};
    poolSizes[0].type            = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    poolSizes[0].descriptorCount = globalSets * 2; // MVP + Lights per set
    ...
    ci.maxSets       = globalSets + static_cast<uint32_t>(FramesInFlight) * 32;
...
void VulkanContext::CreateGlobalDescriptorSets()
{
    ...
    // One descriptor set per (frame, cameraSlot) pair.
    // Index: frame * MaxCamerasPerFrame + slot
    const int totalSets = FramesInFlight * MaxCamerasPerFrame;
    ...
    std::vector<VkDescriptorSetLayout> layouts(totalSets, GlobalDescLayout);
    GlobalDescSets.resize(totalSets);
    ...
    for (int frame = 0; frame < FramesInFlight; ++frame)
    {
        ...
        for (int slot = 0; slot < MaxCamerasPerFrame; ++slot)
        {
            const int idx = frame * MaxCamerasPerFrame + slot;

            VkDescriptorBufferInfo mvpInfo{};
            mvpInfo.buffer = MvpUBOs[frame];
            mvpInfo.offset = static_cast<VkDeviceSize>(slot) * MvpSlotStride;
            mvpInfo.range  = 144; // sizeof(ShaderMVPBuffer)

            std::array<VkWriteDescriptorSet, 2> writes{};
            writes[0].sType           = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
            writes[0].dstSet          = GlobalDescSets[idx];
            ...
            writes[1].dstSet          = GlobalDescSets[idx];
...
void VulkanContext::Shutdown()
{
    ...
    if (OffscreenRenderPass != VK_NULL_HANDLE)
        { vkDestroyRenderPass(Device, OffscreenRenderPass, nullptr); OffscreenRenderPass = VK_NULL_HANDLE; }
