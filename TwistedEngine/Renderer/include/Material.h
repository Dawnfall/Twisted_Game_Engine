...
namespace Twisted
{
	inline constexpr const char* PROP_TYPE_KEY = "type";
	inline constexpr const char* PROP_VALUE_KEY = "value";
	inline constexpr const char* PROPERTIES_KEY = "properties";
	inline constexpr const char* SHADER_KEY = "shader";
	inline constexpr const char* TEXTURE_TYPE_NAME = "tex";
	...
	{
		MaterialValue(ShaderValueType v, const std::string& n, const std::string& en, ShaderPropertyType et, size_t ri) :
			val(v),
			reflectionIndex(ri),
			name(n),
			editorName(en),
		...
		std::string        name;       // GLSL uniform name — used for binding and serialization
		std::string        editorName; // display name shown in the editor
		ShaderPropertyType editorType; // editor widget hint (e.g. Color shows color picker)
		int                version = 0;
	};
	...
	private:
		...
		void AllocateTextureDescSets();
		void FreeTextureDescSets();
	};

	// Forward declarations — implemented in the Assets module (Assets/src/AssetEncoders.cpp)
	YAML::Node encodeTObject(const TObject* obj);
	TObject* decodeTObject(const YAML::Node& node);

	template<>
	inline YAML::Node YamlSerialize<Material>(const Material& material)
	{
		YAML::Node node;
		node[SHADER_KEY] = encodeTObject(material.GetShader());
		...
		for (const auto& mv : material.GetValues())
		{
			...
			std::visit([&](const auto& val) {
				...
				if constexpr (std::is_same_v<T, bool>) {
					...
					entry[PROP_VALUE_KEY] = encodeTObject(val.tex.get());
				} else {
					handled = false;
				}
			}, mv.val);
	...
	{
		if (node[SHADER_KEY])
			material.SetShader(static_cast<Shader*>(decodeTObject(node[SHADER_KEY])));
		...
		for (auto& mv : material.GetValues())
		{
			...
			{
				TextureValue tv;
				tv.tex = WPtr<Texture>(static_cast<Texture*>(decodeTObject(valNode)));
				mv.val = tv;
			}
		}
	}
