...
namespace Twisted
{
	...
	void CTransform::SetParent(CTransform& transform, CTransform* newParent)
	{
	...
	void CTransform::SetParent(CTransform& transform, CTransform* newParent, size_t index)
	{
		//TODO....
	}

	//assumes index is valid [0 , size-1] ; could be optimized
	void CTransform::SetSiblingsIndex(CTransform& transform, size_t newIndex)
	{
		size_t index = transform.GetSiblingsIndex();
		if (newIndex == index)
			return;

		CTransform* parent = transform.GetParent();
		if (parent)
	...
	void CTransform::Unparent(CTransform& transform)
	{
		CTransform* currentParentTransform = transform.GetParent();
