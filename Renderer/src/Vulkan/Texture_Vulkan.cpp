...
#include "Twisted/Rendering/Vulkan/Texture_Vulkan.h"
...
namespace Twisted
{
    ...
    Texture::Texture(const std::string& name)
        : TObject(name), m_backend(new TextureBackend())
    {
    }
    ...
    void Texture::Clear()
    {
        ...
        if (m_backend->Sampler   != VK_NULL_HANDLE) vkDestroySampler(ctx.Device, m_backend->Sampler, nullptr);
        if (m_backend->ImageView != VK_NULL_HANDLE) vkDestroyImageView(ctx.Device, m_backend->ImageView, nullptr);
        if (m_backend->Image     != VK_NULL_HANDLE) vkDestroyImage(ctx.Device, m_backend->Image, nullptr);
        if (m_backend->Memory    != VK_NULL_HANDLE) vkFreeMemory(ctx.Device, m_backend->Memory, nullptr);

        *m_backend = TextureBackend{};
    ...
    void Texture::SetData(const TextureData& data, TextureParams params)
    {
        ...
        ctx.CreateImage(static_cast<uint32_t>(data.Width),
                        ...
                        m_backend->Image, m_backend->Memory);

        // Transition + copy + transition
        ctx.TransitionImageLayout(m_backend->Image, fmt,
                                  VK_IMAGE_LAYOUT_UNDEFINED,
                                  VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
        ctx.CopyBufferToImage(stagingBuf, m_backend->Image,
        ...
        // Image view + sampler
        CreateImageView(ctx.Device, m_backend->Image, fmt, VK_IMAGE_ASPECT_COLOR_BIT, m_backend->ImageView);
        CreateSampler(ctx.Device, params, m_backend->Sampler);
    ...
    void Texture::Resize(const Vec2i& newSize)
    {
        ...
        if (Info.Width == newSize.x && Info.Height == newSize.y && m_backend->IsValid()) return;
        ...
        // Determine Vulkan format + usage from existing Info
        ...
        // Destroy old resources (keep sampler if unchanged)
        if (m_backend->ImageView != VK_NULL_HANDLE) vkDestroyImageView(ctx.Device, m_backend->ImageView, nullptr);
        if (m_backend->Image     != VK_NULL_HANDLE) vkDestroyImage(ctx.Device, m_backend->Image, nullptr);
        if (m_backend->Memory    != VK_NULL_HANDLE) vkFreeMemory(ctx.Device, m_backend->Memory, nullptr);
        ...
        CreateImageView(ctx.Device, m_backend->Image, fmt, aspect, m_backend->ImageView);

        if (m_backend->Sampler == VK_NULL_HANDLE)
            CreateSampler(ctx.Device, Params, m_backend->Sampler);
