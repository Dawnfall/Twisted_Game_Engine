...
namespace Twisted::GL
{
	...
	void ReflectBlockMembersGL(GLuint program, BlockDesc& block, GLuint blockResourceIndex)
	{
		...
		std::vector<GLint> memberUniformIndices(static_cast<size_t>(numActive));
		{
		...
		for (GLint uIndex : memberUniformIndices)
		{
			...
			GLint  vals[3] = {};
			glGetProgramResourceiv(program, GL_UNIFORM, static_cast<GLuint>(uIndex), 3, props, 3, nullptr, vals);

			GLenum glType = static_cast<GLenum>(vals[0]);
			...
			m.arrayCount = static_cast<uint32_t>(arraySize);
			m.offsetBytes = static_cast<uint32_t>(offset);
	...
	ShaderReflection ReflectProgramGL(GLuint program)
	{
		...
		for (GLuint i = 0; i < static_cast<GLuint>(uniformCount); ++i)
		{
			...
			GLenum glType = static_cast<GLenum>(valsA[0]);
			...
			valueDesc.arrayCount = static_cast<uint32_t>(arrSize);
		...
		for (GLuint b = 0; b < static_cast<GLuint>(blockCount); ++b)
		{
			BlockDesc block;
			...
			block.sizeBytes = static_cast<uint32_t>(vals[0]);
	...
	)
	{
		...
		for (size_t i = 0; i < refl.values.size(); ++i)
		{
		...
		for (size_t i = 0; i < refl.texIndices.size(); ++i)
		{
			...
			else
			{
				...
				for (uint32_t k = 0; k < t.arrayCount; ++k)
					units[k] = b.unit + static_cast<GLint>(k);
		...
		for (size_t i = 0; i < refl.blocks.size(); ++i)
		{
			...
			b.bindingPoint = static_cast<GLuint>(binding);
