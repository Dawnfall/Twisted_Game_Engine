...
namespace Twisted::GL
{
	...
	GLuint CompileShaderCode(GLenum shaderType, const char* shaderName, const char* shaderCode)
	{
		...
		auto validateShader = [](GLuint id, [[maybe_unused]] const std::string& name, [[maybe_unused]] GLenum kind)->bool {
			GLint success;
			...
			if (logLength > 0)
			{
				...
				if (success != GL_TRUE)
				{
					TWISTED_ERROR("Error: Failed to compile shader: {} ; {} ; Log: {}", kind, name, infoLog);
					return false;
				}
				else
				{
					TWISTED_WARN("Shader compiled with warnings: {} ; {} ; Log: {}", kind, name, infoLog);
				}
			}
			return true;
			};
