...
namespace Twisted
{
	...
	void Texture::SetData(const TextureData& data, TextureParams params)
	{
		...
		Info.MipLevels = Params.DoMipMaps ? static_cast<uint32_t>(GL::CalcMipCount2D(Info.Width, Info.Height)) : 1;
		...
		glTextureStorage2D(m_backend->TexID, static_cast<GLint>(Info.MipLevels), internalFmt, Info.Width, Info.Height);
