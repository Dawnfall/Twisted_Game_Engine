...
namespace Twisted
{
	inline constexpr const char* WINDOW_CLASS_NAME = "TwistedWin32Window";
	...
	inline static std::pair<Twisted::MouseButton, Twisted::KeyState> win32ToTwistedButton(UINT msg) noexcept
	{
		...
		case WM_MBUTTONUP:
			return { Twisted::MouseButton::Middle ,Twisted::KeyState::RELEASED };
		case WM_XBUTTONDOWN:
		{
	...
	inline static LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
	{
		...
		{
			// VK_SHIFT/CONTROL/MENU are ambiguous — resolve to sided variant via scan code
			WPARAM resolvedVK = wParam;
			if (wParam == VK_SHIFT || wParam == VK_CONTROL || wParam == VK_MENU)
			{
				UINT scanCode = LOBYTE(HIWORD(lParam));
				resolvedVK = MapVirtualKey(scanCode, MAPVK_VSC_TO_VK_EX);
			}

			KeyEvent e;
			e.key = win32ToTwistedKey(resolvedVK);
		...
		case WM_DROPFILES:
		{
			...
			{
				char buf[MAX_PATH];
				if (DragQueryFileA(hDrop, i, buf, MAX_PATH))
					paths.emplace_back(buf);
			}
			DragFinish(hDrop);
			service->DispatchEvent(FilesDroppedEvent{ std::move(paths) });
			return 0;
		}
		case WM_CLOSE:
		{
