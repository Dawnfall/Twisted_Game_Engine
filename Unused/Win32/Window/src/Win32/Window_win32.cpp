...
namespace Twisted
{
	...
	{
		if (m_backend->hwnd)
			SetWindowLongPtr(m_backend->hwnd, GWLP_USERDATA, (LONG_PTR)this);
	}
	...
	WindowBackend::WindowBackend(const std::string& title, Vec2i& size, Vec2i position)
	{
		...
		{
			TWISTED_ERROR("GraphicsContext_win32 (OpenGL) failed: setPixelFormat");
			return;
		}
		...
		if (!glrc)
		{
			TWISTED_ERROR("GraphicsContext_win32 (OpenGL) failed: createContext");
			return;
		}

		TWISTED_INFO("GraphicsContext_win32 (OpenGL) successfully initialized!");
#endif
	...
	void Window::SetVSync([[maybe_unused]] int deltaFrames)
	{
#ifdef TWISTED_BACKEND_OPENGL
	...
	void Window::Clear([[maybe_unused]] const Color& color)
	{
#ifdef TWISTED_BACKEND_OPENGL
	...
	HGLRC createContext(HDC hdc)
	{
		...
		if (wglCreateContextAttribsARB)
		{
			...
			realContext = wglCreateContextAttribsARB(hdc, nullptr, attribs);
			if (!realContext)
			{
		...
		if (!gladLoaded)
		{
			auto loader = [](const char* name) -> void* {
				...
				if (!p) p = reinterpret_cast<void*>(GetProcAddress(GetModuleHandleA("opengl32.dll"), name));
				return p;
			};
